...
export default function ProfileScreen() {
  ...
  const handleShareReferral = async () => {
    ...
    await Share.share({
      message: `Join me on LearnKloud.Today for AWS & GenAI cert prep! Use my referral code ${referral.code}: ${link}`,
      url: link,
    }).catch(() => {});
  };
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <View style={styles.profileTop}>
          ...
          <Text style={[styles.profileName, { color: colors.text, fontSize: t.screenTitle }]}>
            {isGuest ? 'Guest' : (user?.name ?? 'LearnKloud.Today learner')}
          </Text>
        ...
        {isGuest ? (
          <View style={[styles.guestBanner, { backgroundColor: colors.primaryLight, borderColor: colors.primary + '44' }]}>
            ...
            <Pressable
              ...
              accessibilityLabel="Log in to LearnKloud.Today"
        ...
        <View style={[styles.panel, { backgroundColor: colors.surface, borderColor: colors.surfaceBorder }]}>
          ...
          <Pressable
            onPress={() => Alert.alert(
              'Restore Purchases',
              'Coming soon — contact support@learnkloud.today to restore your subscription.',
